...
int	main(void)
{
	...
	AForm *more_taxes = new AForm("Our bureaucrats need bigger cars, More Taxes!");
	AForm *raice_salaries = new AForm("Let's raise the salaries to everyone :D");
	...
	sep(2, "SIGN AFormS");
	try { sam->signAForm(*more_taxes); }
	catch(const std::exception& e) { std::cerr << e.what() << '\n'; }
	try { karen->signAForm(*raice_salaries); }
	catch(const std::exception& e) { std::cerr << e.what() << '\n'; }
